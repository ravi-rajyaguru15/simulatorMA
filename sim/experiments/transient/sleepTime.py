...
def runThread(agent, numEpisodes, results, finished):
	...
	try:
		for e in range(numEpisodes):
			...
			results.put(["Agent %s" % agent.__name__, e, np.average([device.totalSleepTime / device.currentTime.current for device in exp.devices])])
	except:
		debug.printCache()
...
def run(numEpisodes):
	...
	plotting.plotMulti("Sleep Time", results=results, ylabel="Time Asleep %", xlabel="Episode #")  # , save=True)
